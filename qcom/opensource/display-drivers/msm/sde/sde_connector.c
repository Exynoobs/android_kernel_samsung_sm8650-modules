				...
				struct drm_connector *connector)
{
	...
	while ((idx = msm_property_pop_dirty(&c_conn->property_info,
		...
		case CONNECTOR_PROP_BRIGHTNESS:
			...
			SDE_INFO("idx (%d) brightness (%d)\n", idx, c_conn->bl_device->props.brightness);
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
static int ept_prop_bl_level[2] = {-1, -1};
#endif
...
int sde_connector_pre_kickoff(struct drm_connector *connector)
{
	...
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		if (ept_prop_bl_level[vdd->ndx] != -1) {
			/* the backlight level set to property is not set in the next kickoff. */
			if (c_conn->bl_device->props.brightness != ept_prop_bl_level[vdd->ndx]) {
				if (vdd)
					LCD_INFO(vdd, "%d != %d \n",
						c_conn->bl_device->props.brightness, ept_prop_bl_level[vdd->ndx]);
				else
					SDE_INFO("%d != %d \n",
						c_conn->bl_device->props.brightness, ept_prop_bl_level[vdd->ndx]);
				//SDE_DBG_DUMP(0x00, "panic");
				c_conn->bl_device->props.brightness = ept_prop_bl_level[vdd->ndx];
				backlight_update_status(c_conn->bl_device);
			}

			ept_prop_bl_level[vdd->ndx] = -1;
		}
	}
#endif
	...
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		/* SAMSUNG_FINGERPRINT */
		if (vdd && vdd->support_optical_fingerprint) {
...
void sde_connector_helper_bridge_disable(struct drm_connector *connector)
{
	...
	if (display && display->panel) {
		vdd = display->panel->panel_private;
		if (ept_prop_bl_level[vdd->ndx] != -1) {
			LCD_INFO(vdd, "ept_prop_bl_level is %d, initialize  to -1\n", ept_prop_bl_level[vdd->ndx]);
			ept_prop_bl_level[vdd->ndx] = -1;
		}
	}	
#endif
		...
		uint64_t val)
{
	...
	case CONNECTOR_PROP_BRIGHTNESS:
		...
		display = (struct dsi_display *) c_conn->display;
		vdd = display->panel->panel_private;
		ept_prop_bl_level[vdd->ndx] = val;
		LCD_INFO(vdd, "Brightness updated to %d (idx:%d)\n", val, idx);
#else
